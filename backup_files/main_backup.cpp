...
private:
    ...
    void performDynamicLabelPropagation() {
        ...
        std::cout << "Step4: updates_count=" << updates_count << ", BV.size()=" << BV_.size() << std::endl;
    ...
    int performDynamicLabelPropagationGPU() {
        // TODO: CUDA 커널 구현
        std::cout << "  [GPU] CUDA 구현 예정\n";
        return performDynamicLabelPropagationCPU(); // 임시로 CPU 버전 호출
    }
#endif
    ...
    void exchangePartitionUpdates() {
        ...
        std::cout << "Step5: PU_OV=" << PU_.PU_OV.size() << ", PU_ON=" << PU_.PU_ON.size() << ", PU_RO=" << PU_.PU_RO.size() << std::endl;
    ...
    bool checkConvergence() {
        ...
        std::cout << "Step6: edge_rate_=" << edge_rate_ << ", convergence_count_=" << convergence_count_ << std::endl;
